...
begin
    ...
    stim_proc: process begin
        ...
        t_RA1E <= (others => '0'); t_RA2E <= (others => '0'); t_WA3M <= (others => '0'); t_WA3W <= (others => '0'); t_RegWriteM <= '0'; t_RegWriteW <= '0'; t_ALUResultM <= (others => '0'); t_ResultW <= (others => '0');
        wait for 5 ns;
        ...
        t_RA1E <= x"1"; t_RA2E <= x"2"; t_WA3M <= x"3"; t_WA3W <= x"4"; t_RegWriteM <= '1'; t_RegWriteW <= '1'; t_ALUResultM <= x"00000001"; t_ResultW <= x"00000002";
        wait for 5 ns;
        assert (t_ToForwardD1E = '0' and t_ToForwardD2E = '0' and t_ForwardD1E = x"00000002" and t_ForwardD2E = x"00000002") report "Failed HazardUnit Test Case 1" severity error;

        -- Test Case 2: Registers match, but not written, so no forwarding.
        t_RA1E <= x"1"; t_RA2E <= x"2"; t_WA3M <= x"1"; t_WA3W <= x"2"; t_RegWriteM <= '0'; t_RegWriteW <= '0'; t_ALUResultM <= x"00000001"; t_ResultW <= x"00000002";
        wait for 5 ns;
        assert (t_ToForwardD1E = '0' and t_ToForwardD2E = '0' and t_ForwardD1E = x"00000002" and t_ForwardD2E = x"00000002") report "Failed HazardUnit Test Case 2" severity error;

        -- Test Case 3: One register matches with Memory, but not the other, the former is forwarded.
        t_RA1E <= x"1"; t_RA2E <= x"2"; t_WA3M <= x"2"; t_WA3W <= x"3"; t_RegWriteM <= '1'; t_RegWriteW <= '1'; t_ALUResultM <= x"00000001"; t_ResultW <= x"00000002";
        wait for 5 ns;
        assert (t_ToForwardD1E = '0' and t_ToForwardD2E = '1' and t_ForwardD1E = x"00000002" and t_ForwardD2E = x"00000001") report "Failed HazardUnit Test Case 3" severity error;

        -- Test Case 4: One register matches with Writeback, but not the other, the former is forwarded.
        t_RA1E <= x"1"; t_RA2E <= x"2"; t_WA3M <= x"3"; t_WA3W <= x"1"; t_RegWriteM <= '1'; t_RegWriteW <= '1'; t_ALUResultM <= x"00000001"; t_ResultW <= x"00000002";
        wait for 5 ns;
        assert (t_ToForwardD1E = '1' and t_ToForwardD2E = '0' and t_ForwardD1E = x"00000002" and t_ForwardD2E = x"00000002") report "Failed HazardUnit Test Case 4" severity error;

        -- Test Case 5: One register matches with Writeback, one matches with Memory.
        t_RA1E <= x"1"; t_RA2E <= x"2"; t_WA3M <= x"2"; t_WA3W <= x"1"; t_RegWriteM <= '1'; t_RegWriteW <= '1'; t_ALUResultM <= x"00000001"; t_ResultW <= x"00000002";
        wait for 5 ns;
